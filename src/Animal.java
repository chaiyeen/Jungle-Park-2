...
/**
* super class for any animal type in our Jungle Park application.
 * It defines the common features for all these animals as well as a default behavior.
 * class Animal implements also the ParkGUI interface 
 * 		all the methods defined in that interface are overridden in Animal class.
 * @author Mouna Kacem
 */
public class Animal implements ParkGUI {
   ...
   * It sets also its position to the mouse position if the tiger is being dragged (i.e. if its isDragging field is set to true).
   */
  @Override
  public void draw() {
  ...
  private void displayLabel() {
    ...
    this.processing.text(label, this.position[0], this.position[1] + this.image.height / 2 + 4);// display label                                                                                      // text
  }
   ...
   * Is not a particular behavior for this animal because the behavior of each animal will depend on the information
   * about the area around that animal (presence of food or threat for instance)
   * Therefore, this method should be overridden by the subclasses
   */
  public void action() {
    // This method should be overridden by a subclasses
  }  
  
  /* isClose() method Checks if another animal is in its neighborhood
   * @return true if otherAnimal is located within range distance around the current animal 
   * 		 false otherwise
   */
